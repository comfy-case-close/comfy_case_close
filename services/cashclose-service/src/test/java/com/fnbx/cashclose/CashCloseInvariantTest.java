...
/**
 * The invariants of a cash close that can be checked without a database.
 *
 * <h2>Why the drawer identity is not tested here any more</h2>
 * This class used to be {@code DrawerIdentityTest}. Back then {@code cashRemaining}
 * was a column the application wrote, and the test verified it equalled
 * {@code counted - withdrawal - eod - tipsInDrawer}. It is now that expression,
 * computed in {@code cashclose.v_close_calc}. An identity that cannot be violated
 * has nothing to test - restating it in Java would only test the test.
 *
 * <p>What remains worth checking is the four things that replaced it, each a place
 * where bugs actually happen:
 * <ol>
 ...
 * </ol>
 *
 * <p>Figures come from Comfy's real spreadsheet.
 */
class CashCloseInvariantTest {
    ...
    class SignConvention {
        ...
        void expensesNegativeTipsPositive() {
    ...
    class Scd2Boundary {
        ...
        @Test
        @DisplayName("validFrom inclusive, validTo exclusive - changeover day belongs to the new version")
        void boundaryIsHalfOpen() {
    ...
    // ------------------------------------------------------------------------
    // CashCloseCalc maps a view and has no setters, so tests build it by
    // reflection. The other two have setters.
    // ------------------------------------------------------------------------

    private static CashMovement movement(EffectType effect, String signed) {
    ...
    private static MovementKind kind(String from, String to) {
